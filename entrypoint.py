...
def main():
    ...
    df_grafik_today_working_res['Actual finish time'] = pd.to_datetime(
        str((pd.Timestamp("today") + pd.Timedelta(hours=3)).year) + '-' + str(
            (pd.Timestamp("today") + pd.Timedelta(hours=3)).month) \
        + '-' + str((pd.Timestamp("today") + pd.Timedelta(hours=3)).day) + ' ' + df_grafik_today_working_res[
            'Actual finish time'], format='%Y-%m-%d %H:%M:%S')
