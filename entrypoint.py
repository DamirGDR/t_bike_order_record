...
def get_etl_batch_size() -> int:
    return int(os.environ.get("etl_batch_size", "25000"))
...
def _run_alarms_and_telegram(
    ...
    select_fresh_alarms_6 = """
        SELECT 
            ...
            ta.id ,
            IFNULL(tb.`number`, '0') AS number ,
            IFNULL(tc.name, '0') AS city , 
            IFNULL(ta.lat, 0) AS lat,
            IFNULL(ta.lng, 0) AS lng,
            IFNULL(ta.status, 0) AS status,
            IFNULL(ta.`type`, 0) AS type,
            IFNULL(ta.note, '0') AS note,
            IFNULL(ta.assignee_id, 0) AS assignee_id,
            IFNULL(ta.assigned, STR_TO_DATE("2024-01-01 00:00:00", "%Y-%m-%d %H:%i:%s")) AS assigned,
            IFNULL(ta.resolved, STR_TO_DATE("2024-01-01 00:00:00", "%Y-%m-%d %H:%i:%s")) AS resolved,
            IFNULL(ta.modified, STR_TO_DATE("2024-01-01 00:00:00", "%Y-%m-%d %H:%i:%s")) AS modified,
            IFNULL(ta.created, STR_TO_DATE("2024-01-01 00:00:00", "%Y-%m-%d %H:%i:%s")) AS created
