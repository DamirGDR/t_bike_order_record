...
def main():
    ...
    df_grafik_today_working_res['Actual start time'] = pd.to_datetime(str((pd.Timestamp("today") + pd.Timedelta(hours=3)).year) + '-' + str((pd.Timestamp("today") + pd.Timedelta(hours=3)).month).str.zfill(2) \
    + '-' + str((pd.Timestamp("today") + pd.Timedelta(hours=3)).day) + ' ' + df_grafik_today_working_res['Actual start time'], format='%Y-%m-%d %H:%M:%S')
