...
def main():
    ...
    select_fresh_t_admin_log_mysql = '''
            SELECT
                ...
                IfNULL(t_admin_log.func_id,0) AS func_id,
                IfNULL(t_admin_log.data_id,0) AS data_id,
                IfNULL(t_admin_log.date,0) AS date,
                NOW() AS add_time
    ...
    select_fresh_alarms_1 = '''WITH detected_combinations AS (
                ...
                SELECT
                    ...
                    res_tab.imei,
                    res_tab.number,
                    res_tab.detection_time,
                    res_tab.lat,
                    res_tab.lng,
                    res_tab.combination,
                    res_tab.distance_prev_3_min,
                    res_tab.g_lat,
                    res_tab.g_lng,
                    res_tab.first_alarm_location,
                    res_tab.now_location,
                    res_tab.city
                FROM 
                    (SELECT
                        NOW() AS add_time,
                        ...
                        RANK() OVER (PARTITION BY alarm_tab.imei ORDER BY alarm_tab.detection_time DESC) AS rn,
                        t_city."name" AS city
                    FROM 
                        (
                        SELECT
    ...
    select_fresh_alarms_3 = '''
        ...
        FROM 
            (
            SELECT
                to_timestamp(t_bike_use."date") AS start_timestamp,
                t_bike_use.*,
                LEAD(t_bike_use.ride_status) OVER (PARTITION BY t_bike_use.uid ORDER BY t_bike_use.date) as next_status,
                LEAD(t_bike_use.ride_status, 2) OVER (PARTITION BY t_bike_use.uid ORDER BY t_bike_use.date) as next_next_status,
                LEAD(t_bike_use.bid) OVER (PARTITION BY t_bike_use.uid ORDER BY t_bike_use.date) as next_bid,
        ...
        LEFT JOIN damir.t_city ON t_bike.city_id = t_city.id
        WHERE t_bike_use_temp.bid = t_bike_use_temp.next_bid
            AND t_bike_use_temp.ride_status = 7
    ...
    df_grafik['Planned start time'] = df_grafik['Planned start time'].fillna('00:00:00').replace('', '00:00:00')
    df_grafik['Planned finish time'] = df_grafik['Planned finish time'].fillna('00:00:00').replace('', '00:00:00')
    df_grafik['Actual start time'] = df_grafik['Actual start time'].fillna('00:00:00').replace('', '00:00:00')
    df_grafik['Actual finish time'] = df_grafik['Actual finish time'].fillna('00:00:00').replace('', '00:00:00')
    ...
    # df_grafik_today = df_grafik[df_grafik['Date'] == pd.to_datetime(
    #     (pd.Timestamp("today") + pd.Timedelta(hours=3)).strftime("%m/%d/%Y"))].copy()
    # df_grafik_today['Конец дня'] = df_grafik_today['Date'] + pd.Timedelta(days=1) - pd.Timedelta(microseconds=1)
    # df_grafik_today['Worker id'] = df_grafik_today['Worker id'].astype(int)
    #
    # df_grafik_today_working = df_grafik_today[df_grafik_today['Actual start time'].notna()].copy()
    # df_grafik_today_working = df_grafik_today_working[df_grafik_today_working['Actual start time'] != '']
    # df_grafik_today_working['rn'] = df_grafik_today_working.groupby("Worker id")["Actual start time"] \
    #     .rank(method="first", ascending=False)
    # df_grafik_today_working = df_grafik_today_working[df_grafik_today_working['rn'] == 1]
    # df_grafik_today_working_res = df_grafik_today_working.copy()
    #
    # df_grafik_today_working_res['Actual finish time'] = df_grafik_today_working_res.apply(type_stavka, axis=1)
    #
    # df_grafik_today_working_res['Actual finish time'] = pd.to_datetime(str((pd.Timestamp("today") + pd.Timedelta(hours=3)).year) + '-' + str((pd.Timestamp("today") + pd.Timedelta(hours=3)).month).zfill(2) \
    #     + '-' + str((pd.Timestamp("today") + pd.Timedelta(hours=3)).day) + ' ' + df_grafik_today_working_res['Actual finish time'], format='%Y-%m-%d %H:%M:%S')
    #
    # df_grafik_today_working_res['Actual finish time'] = df_grafik_today_working_res.apply(type_stavka, axis=1)
    #
    # # df_grafik_today_working_res['Actual start time'] = pd.to_datetime(df_grafik_today_working_res['Actual start time'])
    # df_grafik_today_working_res['Actual start time'] = pd.to_datetime(str((pd.Timestamp("today") + pd.Timedelta(hours=3)).year) + '-' + str((pd.Timestamp("today") + pd.Timedelta(hours=3)).month).zfill(2) \
    # + '-' + str((pd.Timestamp("today") + pd.Timedelta(hours=3)).day) + ' ' + df_grafik_today_working_res['Actual start time'].astype(str), format='%Y-%m-%d %H:%M:%S')
    #
    # df_grafik_today_working_res['Actual finish time'] = pd.to_datetime(
    #     df_grafik_today_working_res['Actual finish time'])
    #
    # df_temp = df_alarms_5.merge(df_grafik_today_working_res, on='Worker id', how='left')
    #
    # df_temp['Working_grafik'] = df_temp['date'].between(df_temp['Actual start time'], df_temp['Actual finish time'])
    #
    # # df_temp.fillna(0, inplace=True)
    #
    # df_temp = df_temp[['id', 'Worker id', 'date', 'number', 'name', 'Worker username',
    #                    'Worker nickname', 'Working_grafik']].rename(columns={'name': 'city_bike'})
    # df_temp['add_time'] = pd.Timestamp.now() + pd.Timedelta(hours=3)
    #
    # # Запрос из Workers_city_role
    # SPREADSHEET_ID = '1dSOV9X2FV3mnOmnwWvTMJuCCZ-tVBf64DP90k3EYD90'
    # RANGE_NAME = 'Workers_city_role!A:E'
    # df_workers_city_role = read_sheet_data_to_pandas(sheets_service, SPREADSHEET_ID, RANGE_NAME)
    # df_workers_city_role.rename(columns={'id': 'Worker id'}, inplace=True)
    # df_workers_city_role['Worker id'] = df_workers_city_role['Worker id'].astype(int)
    #
    # df_temp = df_temp.merge(df_workers_city_role, how='left', on='Worker id')
    #
    # df_temp['rn'] = df_temp.groupby(['Worker id', 'number', 'Working_grafik'], as_index=False)['date'].rank(method="first",
    #                                                                              ascending=False)
    # df_temp = df_temp[df_temp['rn'] == 1]
    # df_temp = df_temp.drop('rn', axis=1)
    #
    # df_temp.replace('', '0').to_sql("alarms_5", engine_postgresql, if_exists="append", index=False)
    # print('Added {x} records to alarms_5 in Postgres!'.format(x=df_temp.shape[0]))
    #
    #
    #
    ...
    for id in df_unsent_records_5['id']:
        ...
        user_name = df_unsent_records_5.loc[(df_unsent_records_5['id'] == id), 'user_name'].iloc[0]
        nickname = df_unsent_records_5.loc[(df_unsent_records_5['id'] == id), 'nickname'].iloc[0]
